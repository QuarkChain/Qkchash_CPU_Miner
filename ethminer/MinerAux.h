...
public:
	...
	bool interpretOption(int& i, int argc, char** argv)
	{
		...
		else if ((arg == "--work-timeout") && i + 1 < argc)
		{
			...
			m_shardId = "0x" + string(argv[++i]);
		}
		else if ((arg == "--coinbase") && i + 1 < argc) {
			m_coinbase = string(argv[++i]);
		}
...
private:
		...
		while (m_running)
			try
			{
				...
				for (unsigned i = 0; !completed; ++i)
				{
					...
					Json::Value v = prpc->eth_getWork(m_shardId, m_coinbase);
					h256 hh(v[0].asString());
					// h256 newSeedHash(v[1].asString());

					long long current_height = std::stoll(v[1].asString(), nullptr, 16);
				...
				//if (EthashAux::eval(current.seedHash, current.headerHash, solution.nonce).value < current.boundary)
				//{
					bool ok = prpc->eth_submitWork(m_shardId, "0x" + toString(current.headerHash), "0x" + toString(solution.nonce), "0x" + toString(solution.mixHash));
					if (ok) {
				...
				//{
				//	bool ok = prpc->eth_submitWork(m_shardId, "0x" + toString(previous.headerHash), "0x" + toString(solution.nonce), "0x" + toString(solution.mixHash));
				//	if (ok) {
				//		cnote << "B-) Submitted and accepted.";
				...
				//else {
				//	f.failedSolution();
				//	cwarn << "FAILURE: CPU gave incorrect result!";
				//}
				current.reset();
			}
	...
	string m_farmURL = "54.202.245.214";
	string m_farmFailOverURL = "54.202.245.214";
	...
	int m_stratumClientVersion = 2;
	int m_stratumProtocol = STRATUM_PROTOCOL_ETHPROXY;
	//string m_user = "0x2f6C49AeB91b931E0b9e9a158B328796976fA49A";
	string m_user;
	string m_pass;
	string m_port;
	//string m_port = "8078";
	string m_fuser = "0x2f6C49AeB91b931E0b9e9a158B328796976fA49A";
	string m_fpass = "";
	string m_email = "";
	string m_fport = "8078";
	string m_shardId = "0x10001";
	string m_coinbase = "0x2f6C49AeB91b931E0b9e9a158B328796976fA49A";
