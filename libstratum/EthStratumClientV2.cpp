...
void EthStratumClientV2::processReponse(Json::Value& responseObject)
{
	...
	switch (id)
	{
		...
		//Change according to QuarkChain requirement
			...
			if (params.isArray())
			{
				...
				else
				{
					...
					if (sHeaderHash != "" && sSeedHash != "" && sShareTarget != "")
					{
                        ...
                        long long current_height = std::stoll(sSeedHash, nullptr, 16);
						h256 seedHash = EthashAux::seedHash((unsigned) current_height);

						//cnote << sHeaderHash << "sHeaderHash";
						//cnote << sSeedHash << "height";
                        //cnote << seedHash << "sSeedHash";
                        //cnote << sShareTarget << "sHareTarget";
...
bool EthStratumClientV2::submit(EthashProofOfWork::Solution solution) {
    ...
    //cnote << EthashAux::eval(tempWork.seedHash, tempWork.headerHash, solution.nonce).value << "value";
	cnote << tempWork.boundary << "bounday";
	...
	case STRATUM_PROTOCOL_STRATUM:
		...
		break;
	case STRATUM_PROTOCOL_ETHPROXY:
		json = "{\"id\": 4, \"worker\":\"" + m_worker + "\", \"method\": \"eth_submitWork\", \"params\": [\"0x" + tempWork.headerHash.hex() + "\",\"0x" + solution.nonce.hex() + "\",\"0x" + solution.mixHash.hex() + "\"]}\n";
		break;
	case STRATUM_PROTOCOL_ETHEREUMSTRATUM:
	...
	else if (EthashAux::eval(tempPreviousWork.seedHash, tempPreviousWork.headerHash, solution.nonce).value < tempPreviousWork.boundary)
	{
		...
		case STRATUM_PROTOCOL_STRATUM:
			...
			break;
		case STRATUM_PROTOCOL_ETHPROXY:
			json = "{\"id\": 4, \"worker\":\"" + m_worker + "\", \"method\": \"eth_submitWork\", \"params\": [\"0x" + tempPreviousWork.headerHash.hex() + "\",\"0x" + solution.nonce.hex() + "\",\"0x" + solution.mixHash.hex() + "\"]}\n";
			break;
		case STRATUM_PROTOCOL_ETHEREUMSTRATUM:
